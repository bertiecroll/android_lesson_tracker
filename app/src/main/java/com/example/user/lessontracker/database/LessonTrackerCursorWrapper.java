...
public class LessonTrackerCursorWrapper extends CursorWrapper {
    ...
    public Lesson getLesson() {
        ...
        long duration = getLong(getColumnIndex(LessonTable.Cols.DURATION));
        String notes = getString(getColumnIndex(LessonTable.Cols.NOTES));

        Lesson lesson = new Lesson(id, cohortId, topicId, date, taught, duration, notes);
        return lesson;
    }
    ...
    public Tag getTag() {
        ...
        String type = getString(getColumnIndex(TagTable.Cols.TYPE));

        Tag tag = new Tag(id, iconResourceId, title, type);
        return tag;
    }
